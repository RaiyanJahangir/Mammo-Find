...
document_embeddings = embedding_model.encode(datasets, convert_to_numpy=True)
...
def retrieve_documents(query, top_k=1):
    ...
    retrieved_docs = [datasets[i] for i in indices[0]]
...
# results=retrieve_documents("What is the article associated with EMBED dataset?", 1)
...
The users will ask you different questions about datasets like which dataset is suitable for which task, which dataset is accessible, comparisons and so on.
...
def handle_terminal_chat():
    ...
    while True:
        ...
        print("===================")
        print("===================")
        retrieved_documents = retrieve_documents(user_input, top_k=1)  # Retrieve related data
        ...
        result = chain.invoke({
            ...
            "document_context": retrieved_documents,
            "question": user_input
        })
