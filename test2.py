...
def ensure_model_installed(model_name):
    ...
    """
    try:
        # List locally installed models (adjust command if necessary)
...
def chat():
    ...
    model_name = data.get('model') or "llama3.2:3b-instruct-q4_K_M"  # Use provided model or fallback default
    prompt_type = data.get('prompt_type') or "zero_shot"  # Use provided prompt type or fallback default
    if user_message:
        print(f"Using model: {model_name}\n")
        # Check and install the model if necessary
        ensure_model_installed(model_name)
        
        response = query_rag(user_message, model_name, prompt_type)
        print(f"AI: {response}\n")
        # Run graph generation in a background thread
